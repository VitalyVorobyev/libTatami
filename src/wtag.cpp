...
const double WTag::m_wbins[8] = {0.0,0.1,0.25,0.5,0.625,0.75,0.875,1.01};
  ...
  while(!ifile.eof()){
    ...
    if(3 != sscanf(line.c_str(),"%lf %lf %lf",&val,&errp,&errn)){
      if(1 != sscanf(line.c_str(),"%lf",&val)){
        cout << "WTag::LoadParameters: can't parse string " << line << endl;
        continue;
      }
    }
    m_w.push_back(val);
...
double WTag::Delut(const int bin){
  if(bin<0 || bin>=(int)m_w.size()){
...
double WTag::Delut(const double& q){
  return Delut(GetBin(q));
}

int WTag::GetBin(const double& q){
  int i=0;
  while(m_wbins[i+1] < fabs(q)) i++;
  return i;
}
